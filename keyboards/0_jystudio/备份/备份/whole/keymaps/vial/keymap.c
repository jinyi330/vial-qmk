...
#include "./lib/bongocat/bongocat.c"
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[_BASE] = LAYOUT(
		...
		KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, RAISE,
		KC_Z, KC_X, KC_C, KC_V, LOWER, KC_B, KC_N, KC_M,
...
// #define ATEST dynamic_keymap_get_keycode(biton32(state),2,6)
// #define BTEST dynamic_keymap_get_keycode(biton32(state),2,4)
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    if (scrolling_mode) {
        ...
        if (mouse_report.x < 0) {
            tap_code16(dynamic_keymap_get_keycode(0,3,1));
        }
        if (mouse_report.y > 0) {
            tap_code16(dynamic_keymap_get_keycode(0,3,2));
        }
        if (mouse_report.y < 0) {
            tap_code16(dynamic_keymap_get_keycode(0,3,0));
        }
